// FYPro — Telegram bot webhook
// POST /api/telegram-webhook — receives Telegram updates, dispatches commands.
// Security: silently ignores messages from any chat other than TELEGRAM_CHAT_ID.
// Always returns 200 immediately so Telegram does not retry.
...
async function sendReply(chatId, text) {
  ...
  try {
    ...
    console.error('[telegram-webhook] sendReply failed:', err.message);
  }
}
...
async function cmdHealth() {
  ...
  const [sbResult, redisResult, usageResult] = await Promise.all([
  ...
  const sbStatus    = sbResult ? '🟢 OK' : '🔴 Error';
  const redisStatus = redisResult === null ? '⚠️ Not configured'
                    : redisResult           ? '🟢 OK'
                    :                        '🔴 Error';

  const spent = parseFloat(usageResult.data?.total_cost_usd || 0);
  const pct   = cap > 0 ? spent / cap : 0;
...
export default async function handler(req, res) {
  // Telegram expects 200 quickly — respond before doing any work.
  res.status(200).end();
  ...
  try {
    ...
    else return; // silently ignore unknown messages
